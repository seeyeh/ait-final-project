...
const getWorkout = asyncHandler(async (req, res) => {
  ...
  let workouts = await Workout.find(req.query)
    .populate({ path: 'activities.exercise', select: 'name' })
    .exec();
...
const createNewWorkout = asyncHandler(async (req, res) => {
  ...
  if (!parentUser || !Array.isArray(activities) || activities.length === 0) {
    return res
      .status(400)
      .json({ message: 'parentUser and activities array are required' });
  }
  ...
  for (let activity of activities) {
    // create attempt in both workout and exercise
    const { result, error } = await createAttempt(parentUser, activity);
    if (error)
      return res
        .status(400)
        .json({ message: `Invalid activities array: '${error}'` });
...
const updateWorkout = asyncHandler(async (req, res) => {
  ...
  if (!parentUser || !id)
    return res
      .status(400)
      .json({ message: 'parentUser and workout id fields are required' });
  ...
  for (let patch of patches) {
    ...
    if (
      path in WorkoutSchemaFields === false ||
      path === '_id' ||
      path === 'parentUser' ||
      path === 'date'
    )
    ...
    const { result, error } = await patchWorkout(
      workout,
      parentUser,
      op,
      path,
    ...
    if (error)
      return res
        .status(400)
        .json({ message: `Error (${completedPatches} completed): ${error}` });
    workout[path] = result;

    // save per workout to preserve consistency with exercises if error happens
    await workout.save();
    completedPatches++;
  }
...
const deleteWorkout = asyncHandler(async (req, res) => {
  ...
  // remove workout attempts from respective exercises
  for (let { _id } of workout.activities) {
    const { error } = await removeAttempt(parentUser, _id.toString(), workout);
    if (error)
      return res
        .status(400)
        .json({ message: `Error deleting activities: ${error}` });
  }

  const { deletedCount } = await workout.deleteOne();
  if (deletedCount !== 1)
    return res.status(400).json({ message: `Workout could not be deleted` });
  res.json({ message: `Workout ${workout.name} deleted` });
});

async function patchWorkout(workout, parentUser, op, path, value) {
  const isIterablePath =
    Array.isArray(workout[path]) || workout[path] instanceof Map;
  switch (op) {
    case 'add':
      // validation: add operation must add to an iterable
      if (!isIterablePath)
        return { error: `Invalid patch path, must be iterable` };

      if (path === 'stats') {
        // map data type requires key and value, add operation is same as replace
        if (!value.key || !value.value)
          return {
            error: `Invalid patch value for stats: key, value fields required`
          };
        workout[path].set(value.key, value.value);
      } else {
        // path === 'activities'
        const { result, error } = await createAttempt(
          parentUser,
          value,
          workout
        );
    ...
    case 'remove':
      ...
      if (!isIterablePath)
        return { error: `Invalid patch path, must be iterable` };
      if (path === 'stats') {
        // map must contain specified key
        if (!value.key || !workout[path].has(value.key))
          return {
            error: `Invalid patch value for stats: key in stats required`
          };
        workout[path].delete(value);
      } else {
        // path === 'activities'
        const { result, error } = await removeAttempt(
          parentUser,
          value,
          workout
        );
        if (error)
          return { error: `Invalid patch value for activities: ${error}` };
        workout[path].splice(result, 1);
      }
      break;

    // replace operation for name, journal
    case 'replace':
      // validation: replace operation cannot be used on iterable and value must be string
      if (isIterablePath)
        return { error: `Invalid patch path, cannot replace an iterable` };
...
async function createAttempt(parentUser, activity, workout) {
  ...
  for (let field in activity) {
    if (field in AttemptSchemaFields == false)
      return { error: `Invalid activity` };
  }
...
async function removeAttempt(parentUser, activityId, workout) {
  // get array index of attempt to remove from workout
  let activityIndex = workout.activities
  ...
  // delete attempt from exercise
  const exercise = await Exercise.findOne({
    parentUser,
    _id: activity.exercise
  }).exec();
  if (exercise) {
    exercise.history = exercise.history.filter(
